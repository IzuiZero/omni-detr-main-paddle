...
"""
from paddle.vision.datasets import VisionDataset
...
class CocoDetection_semi(VisionDataset):
    """`MS Coco Detection <http://mscoco.org/dataset/#detections-challenge2016>`_ Dataset.
    Args:
        root (string): Root directory where images are downloaded to.
        annFile (string): Path to json annotation file.
    ...
    def __getitem__(self, index):
        ...
        path = coco.loadImgs(img_id)[0]['file_name']
        indicator = coco.loadImgs(img_id)[0]['indicator']
        labeltype = coco.loadImgs(img_id)[0]['label_type']

        img = self.get_image(path)
        if self.transforms is not None:
            img, target = self.transforms(img, target)

        return img, target, indicator, labeltype
