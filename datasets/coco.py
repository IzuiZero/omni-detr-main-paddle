...
class ConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target):
        ...
        points = paddle.to_tensor(points, dtype='float32').reshape([-1, 2])
...
def build_semi_label(image_set, args):
    ...
    if args.dataset_file == "voc_semi":
        ...
        PATHS = {
            "train": (root / "unlabeled2017", root / "annotations" / args.annotation_json_label),
        ...
        PATHS = {
            "train": (root / "train2017", root / "annotations" / args.annotation_json_label),
            "val": (root / "val2017", root / "annotations" / f'{mode}_w_indicator_val2017.json'),
        }

    img_folder, ann_file = PATHS[image_set]
    if args.pixels == 600:
        dataset = CocoDetection_semi(img_folder, ann_file, transforms_strong=make_coco_strong_transforms_with_record600(image_set),
                                     transforms_weak=make_coco_weak_transforms_with_record600(image_set),
                                     return_masks=args.masks,
                                     cache_mode=args.cache_mode, local_rank=get_local_rank(), local_size=get_local_size())
    elif args.pixels == 800:
        dataset = CocoDetection_semi(img_folder, ann_file, transforms_strong=make_coco_strong_transforms_with_record(image_set),
