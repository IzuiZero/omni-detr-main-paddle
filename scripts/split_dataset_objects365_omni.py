...
def main():
    ...
    data_set = 'objects365_train_sampled'
    ...
    sup_class_id = [class_index_list[i] for i in list(np.where(histogram ==
        0)[0])]
    for i_category in sup_class_id:
        imgIds_i = coco_api.getImgIds(catIds=i_category)
        sample_ids.append(imgIds_i[0])
    sample_ids = sorted(sample_ids)
    imgs = coco_api.loadImgs(sample_ids)
    for i_img in imgs:
        i_img['indicator'] = 1
        i_img['label_type'] = fully_labeled
    dataset_anns = [coco_api.imgToAnns[img_id] for img_id in sample_ids]
    anns = [ann for img_anns in dataset_anns for ann in img_anns]
    ith = 0
    for i_ann in anns:
    ...
    output_file_label = (
        '{}{}_objects_omni_label_seed{}_{}fully{}Unsup{}tagsU{}tagsK{}pointsU{}pointsK{}boxesEC{}boxesU.json'
    ...
    output_file_unlabel = (
        '{}{}_objects_omni_unlabel_seed{}_{}fully{}Unsup{}tagsU{}tagsK{}pointsU{}pointsK{}boxesEC{}boxesU.json'
        .format(root_dir, data_set, random_seed, round(100 * fully_labeled),
        round(100 * Unsup), round(100 * tagsU), round(100 * tagsK), round(
        100 * pointsU), round(100 * pointsK), round(100 * boxesEC), round(
        100 * boxesU)))
